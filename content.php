...
<head>
    ...
    <style>
        ...
        .notif-item { 
            padding: 15px 35px 15px 20px; /* Extra padding for close btn */
...
<body>
                                ...
                                <tr id="art-row-<?= $art['id'] ?>">
                                    ...
                                    <td>
                                        <div class="actions">
                                            ...
                                            <button class="btn-icon delete" onclick="deleteArtwork(<?= $art['id'] ?>, this)"><i class="fas fa-trash"></i></button>
                                        </div>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                ...
                                <tr id="srv-row-<?= $srv['id'] ?>">
                                    ...
                                    <td>
                                        <div class="actions">
                                            ...
                                            <button class="btn-icon delete" onclick="deleteService(<?= $srv['id'] ?>, this)"><i class="fas fa-trash"></i></button>
                                        </div>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                ...
                                <tr id="evt-row-<?= $evt['id'] ?>">
                                    ...
                                    <td>
                                        <div class="actions">
                                            ...
                                            <button class="btn-icon delete" onclick="deleteEvent(<?= $evt['id'] ?>, this)"><i class="fas fa-trash"></i></button>
                                        </div>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                                ...
                                <tr id="artist-row-<?= $artist['id'] ?>">
                                    ...
                                    <td>
                                        <div class="actions">
                                            ...
                                            <button class="btn-icon delete" onclick="deleteArtist(<?= $artist['id'] ?>, this)"><i class="fas fa-trash"></i></button>
                                        </div>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
    ...
    <script>
        ...
        function deleteArtwork(id, btn) {
            if(confirm('Are you sure you want to move this artwork to the trash?')) {
                const formData = new FormData(); formData.append('id', id);
                fetch(`artworks.php?action=delete`, { method: 'POST', body: formData })
                    .then(res => res.json())
                    .then(data => { 
                        if(data.success) {
        ...
        function deleteService(id, btn) {
            if(confirm('Delete this service?')) {
                const formData = new FormData(); formData.append('id', id);
                fetch(`services.php?action=delete`, { method: 'POST', body: formData })
                    .then(r=>r.json()).then(d=>{ if(d.success) btn.closest('tr').remove(); });
            }
        }
        ...
        eventForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const data = await res.json();
                if(data.success) location.reload(); else alert('Error saving event');
        ...
        function deleteEvent(id, btn) {
            if(confirm('Delete this event?')) {
                const formData = new FormData(); formData.append('id', id); formData.append('action', 'delete');
                fetch('manage_events.php', { method: 'POST', body: formData })
                    .then(r=>r.json()).then(d=>{ if(d.success) btn.closest('tr').remove(); });
            }
        }
        ...
        artistForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const data = await res.json();
                if(data.success) location.reload(); else alert('Error saving artist');
            } catch(err) { alert('Request failed'); }
        });
        ...
        function deleteArtist(id, btn) {
            if(confirm('Delete this artist?')) {
                const formData = new FormData(); formData.append('id', id); formData.append('action', 'delete');
                fetch('manage_artists.php', { method: 'POST', body: formData })
                    .then(r=>r.json()).then(d=>{ if(d.success) btn.closest('tr').remove(); });
            }
        }
    </script>
