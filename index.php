    ...
    try {
        if ($action === 'send_reset_otp') {
            ...
            if ($res->num_rows > 0) {
                ...
                // Pretend success for security (or show error if preferred)
        ...
        elseif ($action === 'reset_password') {
            ...
            if ($new_pass !== $confirm_pass) {
                ...
                // Verify OTP again before updating (Double Check)
                $stmt = $conn->prepare("SELECT id FROM users WHERE email = ? AND reset_token_hash = ? AND reset_token_expires_at > NOW()");
                $stmt->bind_param("ss", $email, $otp_hash);
                $stmt->execute();
...
if (isset($_POST['login'])) {     
    ...
    if ($result && mysqli_num_rows($result) === 1) {
        ...
        if (!empty($row['account_activation_hash'])) {
            ...
            header("Location: verify_otp"); // UPDATED: Removed .php
            exit();
        }
        if (password_verify($password, $row['password'])) {
            ...
            // UPDATED: Removed .php from redirects
    ...
    if(isset($_SESSION['error_message'])) {
        header("Location: ./?login=1"); // UPDATED: Clean Root Redirect
        exit();
    }
} 
...
if (isset($_POST['sign'])) {
    ...
    if(isset($_SESSION['error_message'])) {
        header("Location: ./?signup=1"); // UPDATED: Clean Root Redirect
        exit();
    }
}
...
<head>
    ...
    <style>
        ...
        .profile-pill {
            ...
            background: rgba(255, 255, 255, 0.1); padding: 4px 15px 4px 4px;
        ...
        .user-dropdown .dropdown-content { top: 140%; right: 0; min-width: 180px; }
        ...
        @media (max-width: 768px) {
            .wave-wrapper { height: auto; padding: 40px 0; }
            .wave-parent { flex-direction: column; gap: 20px; }
            .wave-card { width: 90%; height: 300px; transform: none !important; filter: none !important; }
            .wave-card:hover, .wave-card.open { width: 95%; transform: scale(1.02) !important; }
            .wave-card .content { opacity: 1; }
        }
        ...
        /* === FRIENDLY MODAL UPDATES === */
        /* Redesigned Small Modal (Login/Signup) */
        .modal-card.small { 
            width: 420px; max-width: 95%; padding: 45px 35px; 
        ...
        .modal-header-icon {
            font-size: 3rem; color: var(--accent-orange); margin-bottom: 20px;
            background: rgba(243, 108, 33, 0.1); width: 80px; height: 80px;
        ...
        /* Friendly Input Fields with Icons */
        .friendly-input-group { position: relative; margin-bottom: 20px; text-align: left; }
        .friendly-input-group i {
            position: absolute; top: 50%; left: 20px;
            transform: translateY(-50%); color: #bbb;
            font-size: 1.1rem; pointer-events: none; transition: 0.3s;
        }
        .friendly-input-group input {
            width: 100%; padding: 14px 14px 14px 55px; /* Space for icon */
            border-radius: 50px; /* Pill shape */
            background: #f8f9fa; border: 1px solid #e9ecef;
            font-size: 0.95rem; transition: all 0.3s ease;
            outline: none;
        }
        .friendly-input-group input:focus {
            background: #fff; border-color: var(--accent-orange);
            box-shadow: 0 4px 15px rgba(243, 108, 33, 0.15);
        }
        ...
        /* Regular form styling for other modals (Contact/Reservation) */
...
<body>
            ...
            <div class="latest-grid">
                ...
                <?php else: ?>
                    <?php foreach ($artworks as $art): 
                        // Logic: If status is 'Sold' OR there is a linked completed booking, treat as Sold
    ...
    <div class="modal-overlay" id="loginModal">
        <div class="modal-card small">
            ...
            <form action="./" method="POST"> <div class="friendly-input-group">
    ...
    <script>
        ...
        document.getElementById('forgotForm').addEventListener('submit', function(e) {
            ...
            fetch('./', { method: 'POST', body: formData }) // UPDATED: Clean Fetch (Root)
            .then(r => r.json())
            .then(data => {
        ...
        document.getElementById('newPasswordForm').addEventListener('submit', function(e) {
            ...
            const otp = document.getElementById('otpCode').value; // Need otp to verify hash again
            const btn = this.querySelector('button');
            const originalText = btn.innerHTML;
        ...
        window.toggleFavorite = function(btn, id) {
            ...
            fetch('favorites', { method: 'POST', body: formData }); // UPDATED: Clean Fetch
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if(inquiryForm) {
                inquiryForm.addEventListener('submit', function(e) {
                    ...
                    fetch('inquire', { // UPDATED: Clean Fetch
                        method: 'POST',
                        body: formData
                    })
            ...
            if (notifBtn) {
                ...
                function fetchNotifications() {
                    fetch('fetch_notifications') // UPDATED: Clean Fetch
                        .then(res => res.json())
                        .then(data => {
                            if (data.status === 'success') {
                                ...
                                if (data.notifications.length === 0) {
                                    ...
                                    data.notifications.forEach(notif => {
                                        ...
                                        item.addEventListener('click', (e) => {
                                            ...
                                            fetch('mark_as_read', { method: 'POST', body: formData }) // UPDATED: Clean Fetch
                                        ...
                                        item.querySelector('.btn-notif-close').addEventListener('click', (e) => {
                                            ...
                                            const formData = new FormData();
                                            formData.append('id', notif.id);
                                            fetch('delete_notifications', { method: 'POST', body: formData }) // UPDATED: Clean Fetch
                                                .then(res => res.json())
                ...
                if (markAllBtn) {
                    markAllBtn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        fetch('mark_all_as_read', { method: 'POST' }) // UPDATED: Clean Fetch
                            .then(() => fetchNotifications());
                    });
                }
